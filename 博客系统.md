...
在父项目中使用dependencyManagement管理依赖，并不会引入依赖，而是对依赖进行版本控制，等到子项目引入相应的依赖后就不需要填写版本，进行了版本控制。 [ccblog.sql](C:\Users\PC\Desktop\ccblog.sql) 
...
public class MyApplicationListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent contextRefreshedEvent) {
        //新建触发器键
        TriggerKey key = TriggerKey.triggerKey("trigger","trigger");
        try {
            //通过调度器获取触发器
            ...
            JobDetail jobDetail = JobBuilder
                    ...
                    .newJob(CCJob.class)
                    //任务名称和任务组
                    .usingJobData("name","cc")
                    .withIdentity("ccTask","ccGroup")
                    .build();
...
目前已经接触到了个任务启动类（即应用启动之后，执行某些自定义逻辑）

1.使用对象初始化时机{}，把对象交给bean容器管理，这样容器初始化就会实例对象，并执行对象创建逻辑

2.集成commandLineRunner接口，容器启动之后，就会执行run方法的逻辑

3.自定义starter启动类（未学习）

4.使用应用上下文监听器，集成ApplicationListener接口，重写onApplicationEvent接口，当某个事件执行的时	   候，就会触发相应的逻辑，这里可以选择监听ContextRefreshedEvent上下文重载事件，即任务启动

5.Bean对象afterpropertiesSet方法，也是交给spring容器管理的对象的生命周期方法

6.Bean对象的生命周期包含创建——使用——销毁，Spring可以配置Bean对象在创建和销毁时自动执行的方法：

在bean对象上方使用@postconstruct注解，定义初始化方法使用@predestory注解，定义销毁时机方法，也是交给spring容器管理的对象的生命周期方法，只不过是不同的生命周期方法。
